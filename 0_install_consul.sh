...
kubectl get svc consul-expose-servers -n consul  --context $dc1
kubectl cluster-info --context $ap1
# update the helm files with the consul exporse servers and k8s control plane ( lines 36 & 38 in adminPartition.yaml)
kubectl config use-context $ap1
...
# update the helm files with the consul exporse servers and k8s control plane ( lines 36 & 38 in ap1.yaml)
kubectl config use-context $ap3

## Update the Admin Partition name in the yaml file before installing - should be line 13 
# Install Consul
